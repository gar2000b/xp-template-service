...
A lightweight **experimental** Spring Boot microservice template for building new services across various research and prototype systems.  
...
Run:
```bash
java -jar target/xp-template-service-*.jar
```
...
Manual build:
```bash
docker build -t gar2000b/xp-template-service:latest .
```

Run container:
```bash
docker run -p 8080:8080 gar2000b/xp-template-service:latest
```
